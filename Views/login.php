 <form class="container mt-5 ml-5" action="/index.php?page=login" method="POST">
    <br>
    <input type="email" placeholder="Email" name="email" value="<?= htmlspecialchars_decode($email ?? '')?>">
    <?php if ($msgError['errors']['login']['email']) : ?>
        <p class="text-danger"><?= htmlspecialchars_decode($msgError['errors']['login']['email']) ?></p>
    <?php endif; ?>
    <br>
    <br>
